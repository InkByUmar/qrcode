...
/**
 * Advanced Professional Logo: WhatsApp Green Box with Black/White QR
 */
...
const AdsterraSkyscraperBanner = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={cn("flex flex-col items-center animate-reveal", className)}>
      <div className="text-[9px] font-black text-foreground/20 uppercase tracking-[0.3em] mb-3 text-center">Sponsored Content</div>
      <div 
        ref={containerRef}
        className="w-[160px] h-[600px] bg-secondary border border-border rounded-2xl flex items-center justify-center overflow-hidden"
      >
        <div className="flex flex-col items-center gap-2 text-foreground/10">
...
const AdsterraNativeBanner = ({ className, id }: { className?: string, id: string }) => {
  ...
  return (
    <div className={cn("container mx-auto px-6 animate-reveal", className)}>
      <div className="text-[10px] font-black text-foreground/20 uppercase tracking-[0.3em] mb-4 text-center">Recommended For You</div>
      <div 
        ref={containerRef}
        id={id}
        className="min-h-[160px] md:aspect-[4/1] bg-secondary border border-border rounded-[2.5rem] flex items-center justify-center overflow-hidden relative"
      >
        <div id="container-8a0d2340102217c81755459d2df8b6d0" className="w-full flex items-center justify-center"></div>
        <div className="flex flex-col items-center gap-3 text-foreground/10 absolute pointer-events-none">
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen premium-gradient selection:bg-primary/20 selection:text-foreground overflow-x-hidden">
      {/* NAVIGATION */}
      <header className="fixed top-0 left-0 right-0 z-[100] w-full border-b border-border bg-background/80 backdrop-blur-2xl animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {navItems.map((item) => (
              <a 
                ...
                className="text-[10px] font-black uppercase tracking-widest text-foreground/50 hover:text-primary transition-all duration-300 hover:-translate-y-0.5"
              >
                {item.label}
              </a>
            ))}
          ...
          <div className="flex items-center gap-2 md:gap-4">
             ...
             <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
                <SheetTrigger asChild>
                  <button className="lg:hidden w-10 h-10 rounded-xl bg-secondary border border-border flex items-center justify-center text-foreground/70" aria-label="Open Navigation Menu">
                    ...
                    <nav className="flex-1 p-6 flex flex-col gap-6">
                      {navItems.map((item) => (
                        <a 
                          ...
                          className="text-xs font-black uppercase tracking-[0.2em] text-foreground/50 hover:text-primary"
                        >
                          {item.label}
                        </a>
                      ))}
      ...
      <section className="container mx-auto px-6 pt-32 pb-12 md:pt-48 relative text-center">
        <div className="max-w-5xl mx-auto relative z-10 animate-reveal">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-[80px] font-headline font-black mb-10 leading-[1.1] md:leading-[1] tracking-tight text-foreground">
            Professional artistic <br />
            <span className="text-primary italic">QR Code Generator</span>
          </h1>
          <p className="text-lg md:text-2xl text-foreground/60 max-w-4xl mx-auto leading-relaxed font-light mb-12">
            Create high-resolution, branded QR assets for your business. Optimized for <span className="text-foreground font-medium">mobile PWA installation</span> and <span className="text-foreground font-medium">high-volume bulk production</span>.
          </p>
        </div>
      </section>
      ...
      {/* HOW TO USE SECTION */}
      <section id="how-to-use" className="container mx-auto px-6 py-32 border-t border-border scroll-mt-32">
        <div className="text-center mb-24 animate-reveal">
          <h2 className="text-3xl md:text-5xl font-headline font-bold text-foreground mb-6 uppercase tracking-tight">Professional Branding Guide</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-6xl mx-auto">
          {[
            ...
            { step: '04', icon: ArrowRight, title: 'Studio Export', desc: 'Download high-definition PNG or professional SVG vector files for print.' },
          ].map((item, i) => (
            <div 
              key={i} 
              className={cn(
                "glass-card p-8 rounded-[2.5rem] border-border relative group hover:border-primary/20 transition-all hover:-translate-y-2 animate-reveal",
                i === 0 ? "stagger-1" : i === 1 ? "stagger-2" : i === 2 ? "stagger-3" : "stagger-4"
              )}
            >
              <div className="absolute top-6 right-6 text-2xl font-black text-foreground/5 group-hover:text-primary/10 transition-all">{item.step}</div>
              ...
              <h3 className="text-xl font-headline font-bold text-foreground mb-4">{item.title}</h3>
      ...
      <section id="pwa-features" className="container mx-auto px-6 py-32 border-t border-border scroll-mt-32 bg-primary/[0.01]">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div className="space-y-8 animate-reveal">
              ...
              <h2 className="text-3xl md:text-5xl font-headline font-bold text-foreground leading-tight uppercase tracking-tight">
                Branded Studio <br />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="flex gap-4 group">
                  <div className="w-10 h-10 rounded-xl bg-secondary flex items-center justify-center shrink-0 border border-border group-hover:bg-primary/10 transition-colors">
                    <Monitor className="w-5 h-5 text-primary" />
                  </div>
                  <div>
                    <h4 className="text-foreground font-bold text-sm mb-1 uppercase tracking-tight">Desktop Client</h4>
                    <p className="text-foreground/40 text-xs">Run as a native app on Windows, Mac, or Linux systems.</p>
                  </div>
                </div>
                <div className="flex gap-4 group">
                  <div className="w-10 h-10 rounded-xl bg-secondary flex items-center justify-center shrink-0 border border-border group-hover:bg-primary/10 transition-colors">
                  ...
                  <div>
                    <h4 className="text-foreground font-bold text-sm mb-1 uppercase tracking-tight">Instant Sync</h4>
                    <p className="text-foreground/40 text-xs">Access your generator presets instantly on any device.</p>
                  </div>
                </div>
              </div>
            </div>
            <div className="relative flex justify-center lg:justify-end animate-reveal stagger-2">
              ...
              <div className="glass-card p-3 rounded-[3rem] border-border shadow-2xl relative overflow-hidden w-full max-w-[280px] hover:scale-105 transition-transform duration-700">
                <div className="bg-background rounded-[2.25rem] p-1 aspect-[9/16] relative overflow-hidden flex flex-col items-center justify-center border border-border">
                   <div className="absolute top-6 left-1/2 -translate-x-1/2 w-16 h-1 bg-foreground/10 rounded-full" />
                   ...
                   <div className="mt-10 text-center space-y-3 px-8">
                      ...
                      <h4 className="text-xl font-headline font-black text-foreground uppercase tracking-tighter leading-none">
                        <span className="text-foreground">QR</span> <span className="text-primary">CANVAS</span>
                      </h4>
                      <div className="w-full h-[1px] bg-gradient-to-r from-transparent via-foreground/20 to-transparent" />
                      <p className="text-[9px] text-foreground/30 font-medium leading-relaxed uppercase tracking-widest">
      ...
      {/* FAQ SECTION */}
      <section id="faq" className="container mx-auto px-6 py-32 border-t border-border scroll-mt-32">
        <div className="max-w-3xl mx-auto animate-reveal">
          <div className="text-center mb-20">
            <h2 className="text-3xl md:text-5xl font-headline font-bold text-foreground mb-6 uppercase tracking-tight">Knowledge Base</h2>
            <p className="text-foreground/50 text-sm md:text-base font-medium">Frequently asked questions about professional QR code production and branding.</p>
          </div>
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem key={index} value={`item-${index}`} className="border-border bg-secondary/50 rounded-3xl px-8 transition-all hover:bg-secondary">
                ...
                <AccordionContent className="text-foreground/60 pb-8 text-sm md:text-base leading-relaxed">
                  {faq.a}
                </AccordionContent>
              </AccordionItem>
            ))}
      ...
      <section id="policies" className="container mx-auto px-6 py-32 border-t border-border scroll-mt-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-1 gap-10 flex-1">
            {[
              ...
              <div key={i} className="space-y-6 animate-reveal stagger-1 group">
                ...
                <h4 className="text-xl font-headline font-bold text-foreground uppercase tracking-tight">{policy.title}</h4>
      ...
      {/* FOOTER */}
      <footer className="border-t border-border bg-secondary/30 py-20">
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-12">
            <CustomScannerLogo />
            <div className="flex flex-wrap justify-center items-center gap-8">
               <a href="#how-to-use" className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 hover:text-foreground transition-all">Setup Guide</a>
               <a href="#faq" className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 hover:text-foreground transition-all">FAQ</a>
               <a href="#policies" className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 hover:text-foreground transition-all">Privacy Policy</a>
            </div>
          </div>
          <div className="pt-8 border-t border-border text-center space-y-5">
            <div className="flex justify-center gap-10">
              <span className="text-[10px] font-black uppercase tracking-[0.5em] text-foreground/60 animate-fade-pulse stagger-1">SSL SECURE</span>
              <span className="text-[10px] font-black uppercase tracking-[0.5em] text-foreground/60 animate-fade-pulse stagger-2">GDPR COMPLIANT</span>
              <span className="text-[10px] font-black uppercase tracking-[0.5em] text-foreground/60 animate-fade-pulse stagger-3">PWA READY</span>
            </div>
