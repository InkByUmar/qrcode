...
const QrContentTypeSuggesterInputSchema = z.object({
  content: z.string().describe('The content for which to suggest a QR code type.'),
});
export type QrContentTypeSuggesterInput = z.infer<typeof QrContentTypeSuggesterInputSchema>;

const QrContentTypeSuggesterOutputSchema = z.object({
  type: z.enum(['URL', 'Text', 'Phone', 'Email', 'WiFi']).describe('The suggested QR code content type.'),
});
export type QrContentTypeSuggesterOutput = z.infer<typeof QrContentTypeSuggesterOutputSchema>;

export async function suggestQrContentType(input: QrContentTypeSuggesterInput): Promise<QrContentTypeSuggesterOutput> {
...
const qrContentTypeSuggesterFlow = ai.defineFlow(
  ...
  async input => {
    ...
    if (!output) throw new Error('Suggestion failed');
    return output;
  }
);
